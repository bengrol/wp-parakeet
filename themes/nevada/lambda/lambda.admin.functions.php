...
function lambda_pttiny_init(){
    ...
    if( isset( $wpdb->last_result[0]->ID ) || $_GET['post'] ) {
        
        $id = isset($wpdb->last_result[0]->ID) ? $wpdb->last_result[0]->ID : '';
        $id = empty( $id ) && !empty($_GET['post']) ? (int) $_GET['post'] : $id;
    ...
    if( function_exists( 'icl_object_id' ) && !$script_loaded ) {
