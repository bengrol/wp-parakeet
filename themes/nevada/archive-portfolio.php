...
<?php
			...
			if(is_array($taxonomys)) {
				...
				foreach ($taxonomys as $key => $value ) {
					...
					if($taxonomys[$key]->parent == 0 && isset($taxonomys[$key]->name) && ( is_array($used_taxonomys) && in_array($taxonomys[$key]->name, $used_taxonomys)) && (is_array($current_projects) && in_array($taxonomys[$key]->slug, $current_projects)) ) {
						echo '<li><a href="#" data-filter="*" class="selected">'.lambda_translate_meta($taxonomys[$key]->name).'</a></li>';
					} 
					if((is_array($used_taxonomys) && in_array($taxonomys[$key]->name, $used_taxonomys)) && $taxonomys[$key]->parent != 0 && (is_array($current_projects) && in_array($taxonomys[$key]->slug, $current_projects)) ) {							
	...
	if (have_posts()) : while (have_posts()) : the_post(); $lambda_meta_data->the_meta();
        ...
        <li class="portfolio-item <?php echo $grid.$itemposition; ?> <?php echo $projecttype; ?> clearfix"> 
								...
								<a href="<?php echo get_permalink(); ?>">
									...
									<div class="hover-overlay">
										<?php if(isset($layout['portfolio_item_title']) && $layout['portfolio_item_title'] != 'on') { ?>
											<h1 class="portfolio-title"><?php echo $title; ?><br /><span><?php echo lambda_translate_meta($projecttypeclean); ?></span></h1>
										<?php } else { ?>
						...
						if($layout['portfolio_item_title'] == 'on') { ?>							
							...
							<h1 class="portfolio-title-below"><a href="<?php the_permalink(); ?>"><?php echo $title; ?></a><br /><span><?php echo lambda_translate_meta($projecttypeclean); ?></span></h1>
