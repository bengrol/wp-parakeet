...
if( !function_exists( 'build_grid_opener' ) ) {
    function build_grid_opener($gridsize, $last, $boxtitle='lnotitle', $overflow=false, $entrycontent=false, $singlebox) {
        ...
        $boxclass = (isset($singlebox['boxclass']) && !empty($singlebox['boxclass']) ) ? ' '.$singlebox['boxclass'].' ' : '';
		...
		$gridopener = '<div class="'.$gridvalues[$gridsize].$last.$entrycontent.$activate_desktop.$activate_landscape.$activate_mobile.$boxclass.'  clearfix" '.$overflow.'>';
...
if( !function_exists( 'build_article_opener' ) ) {
    function build_article_opener($gridsize, $last, $boxtitle='lnotitle', $overflow=false, $entrycontent=false, $singlebox) {
		...
		$gridopener = '<section class="'.$gridvalues[$gridsize].$last.$entrycontent.$activate_desktop.$activate_landscape.$activate_mobile.$boxclass.' service clearfix" '.$overflow.'>';
...
if( !function_exists( 'lambda_portfolio_columns' ) ) {
	...
	if((isset($portfolio['activate_portfolio']) && $portfolio['activate_portfolio'] == 'on') || $pagebuilder = true) { ?>
            ...
            <?php echo ($counter % $gridcount[$portfoliogrid] == 0) ? '<div class="clear"></div>' : ''; ?>
...
if( !function_exists( 'render_service_column' ) ) {
    function render_service_column($metadata, $servicegrid='one_fourth', $last='', $home = false, $colnumber=1) { 			
				...
				<?php } ?>	
							...
							<?php if(!empty($metadata['col_icon'])) : ?>
					...
					<article class="service">
                        ...
                        <?php if(!empty($metadata['col_headline'])) : ?>
						<h3>
							<?php echo lambda_translate_meta($metadata['col_headline']); ?>
						</h3>
                        <?php endif; ?>
